    ...
    <div class="field" :class="field.type">
        ...
        <div v-else class="langs">
            <div class="indicators">
                <p v-for="(l, key) in lang" :key="key" class="lang" :class="'lang-' + key" v-text="key" @click="$store.dispatch('lang/setFormLang', key)" />
            </div>
            ...
            valid() {
                ...
                if (Array.isArray(this.$refs.field)) {
                    this.$refs.field.forEach(field => {
                        if (!field.valid()) {
                            error = true;
                            this.$store.dispatch("lang/setFormLang", field.$el.dataset.lang);
                        }
                    });
